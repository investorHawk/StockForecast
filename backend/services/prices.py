...
def suggest_stocks(q: str, limit: int = 20) -> List[str]:
    meta = get_schema()
    ...
    if not code_col:
        return []
    q_like = f"%{q}%"
    sql = f"SELECT DISTINCT {_qident(code_col)} as code FROM prices WHERE {_qident(code_col)} LIKE ? ORDER BY 1 LIMIT ?"
    with get_connection(settings.DB_PATH) as conn:
        cur = conn.execute(sql, (q_like, limit))
        rows = cur.fetchall()
